...
export default function FinanceDashboard() {
  ...
  const financialSummary = {
    totalBudget: 150000000, // RAB Awal (150 Juta)
    income: {
      total: 85000000,
      registration: 45000000, // Dari Pendaftaran Tim
      sponsorship: 40000000,  // Dari Sponsor
    },
    expense: {
      total: 32000000,
      pending: 5000000, // Reimbursement yg belum diapprove
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="border-l-4 border-l-blue-500 bg-blue-50/50">
            <CardContent className="p-6">
                <div className="flex items-center justify-between mb-2">
                    <p className="text-sm font-medium text-blue-700">Saldo Saat Ini (Balance)</p>
                    <Wallet className="w-4 h-4 text-blue-600" />
                </div>
                <div className="text-2xl font-bold text-blue-700">Rp {balance.toLocaleString('id-ID')}</div>
                <p className="text-xs text-blue-600 mt-1">Available Cash</p>
            </CardContent>
        </Card>
      </div>

      {/* MONITORING RAB & POS ANGGARAN */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div key={idx} className="space-y-1">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="text-muted-foreground">
                                    {((item.used / item.limit) * 100).toFixed(0)}%
                                </span>
                            </div>
        ...
        <Card>
            ...
            <CardContent className="grid gap-3">
                <Button variant="outline" className="w-full justify-start" onClick={() => window.location.href='/admin/finance/reimbursement-approval'}>
                    <FileText className="w-4 h-4 mr-2" /> Cek Reimbursement (3)
                </Button>
                <Button variant="outline" className="w-full justify-start" onClick={() => window.location.href='/admin/teams'}>
                    <DollarSign className="w-4 h-4 mr-2" /> Verifikasi Pendaftaran (12)
                </Button>
                <Button variant="outline" className="w-full justify-start" onClick={() => window.location.href='/admin/finance/invoices'}>
                    <FileText className="w-4 h-4 mr-2" /> Buat Invoice Sponsor
                </Button>
            </CardContent>
        </Card>
      </div>
